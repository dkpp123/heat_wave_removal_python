...
class HeatWaveUltimateStudio(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("RTX 3090 MASTERING STUDIO v13.0 (Multi-Engine AI)")
    ...
    def setup_ui(self):
        self.header = ctk.CTkLabel(self, text="AI KINEMATIC MIRAGE SUPPRESSION", font=("Impact", 35), text_color="#3498db"); self.header.pack(pady=5)
        ...
        # Tracking Engine Dropdown Option
        lbl_mode = ctk.CTkLabel(t_haze, text="Tracking Engine", font=("Arial", 11, "bold"))
        lbl_mode.grid(row=0, column=0, padx=10, pady=2, sticky="e")
        ToolTip(lbl_mode, "Selects the spatial alignment technique. AI Optical Flow is best for panning/movement. Static is fastest.")
        self.q_mode = ctk.CTkOptionMenu(t_haze, values=["AI Optical Flow (RAFT)", "Global Pan Tracking (FFT)", "Static Camera (None)"])
        self.q_mode.set("AI Optical Flow (RAFT)")
        self.q_mode.grid(row=0, column=1, columnspan=2, padx=10, pady=5, sticky="w")

        # Sliders
        self.h_suppress = self.add_smart_slider(t_haze, "Haze Suppress", 0, 1, 0.90, 1, 0, "Strength of mirage removal.")
        self.h_depth = self.add_smart_slider(t_haze, "Haze Depth", 1, 50, 25, 2, 0, "Pixel vibration sensitivity.")
        self.h_thresh = self.add_smart_slider(t_haze, "Vibration Thresh", 0.1, 5.0, 1.2, 3, 0, "Distinguish haze from motion.")
        self.q_win = self.add_smart_slider(t_haze, "Stable Window", 3, 60, 15, 4, 0, "Frames used to calculate Median.", True)
    ...
    def start_preview(self): 
        ...
        self.preview_display.configure(image=None, text="INITIALIZING TRACKING ENGINE...")
    ...
    def _reader_thread(self, decoder, w, h, limit_frames, free_q, ready_q, pinned_tensors):
        ...
        try:
            for _ in range(limit_frames):
                ...
                ready_q.put(idx)
        except Exception as e: print(f"Reader thread error: {e}")
        finally: ready_q.put(None)
    ...
    def _writer_thread(self, encoder, free_q, ready_q, free_reader_q, out_pinned):
        ...
        except Exception as e: print(f"Writer thread error: {e}")
        finally:
            try: encoder.stdin.close()
            except Exception: pass

    # --- TRACKING ENGINE METHODS ---
    def init_ai_flow(self):
        if hasattr(self, 'flow_model'): return
        from torchvision.models.optical_flow import raft_small, Raft_Small_Weights
        weights = Raft_Small_Weights.DEFAULT
        self.flow_transforms = weights.transforms()
        self.flow_model = raft_small(weights=weights, progress=False).to(self.device).eval()

    def estimate_ai_flow(self, curr_frame, prev_frame):
        """Calculates per-pixel optical flow using Neural Networks (RAFT)"""
        img1 = curr_frame / 255.0
        img2 = prev_frame / 255.0
        ...
        img1, img2 = self.flow_transforms(img1, img2)
        
        with torch.no_grad():
            list_of_flows = self.flow_model(img1, img2)
            flow_lowres = list_of_flows[-1]
            
        if scale < 1.0:
            flow = torch.nn.functional.interpolate(flow_lowres, size=(H, W), mode='bilinear', align_corners=False)
            flow[:, 0, :, :] *= (W / new_W)
            flow[:, 1, :, :] *= (H / new_H)
        else:
            if pad_h > 0 or pad_w > 0: 
                flow = flow_lowres[:, :, :H, :W]
            else: 
                flow = flow_lowres
            
        return flow

    def estimate_global_shift(self, curr_frame, prev_frame):
        """Calculates Fast Global Pan tracking using Fast Fourier Transforms (FFT)"""
        gray1 = 0.299 * curr_frame[:, 0:1] + 0.587 * curr_frame[:, 1:2] + 0.114 * curr_frame[:, 2:3]
        gray2 = 0.299 * prev_frame[:, 0:1] + 0.587 * prev_frame[:, 1:2] + 0.114 * prev_frame[:, 2:3]
        
        f1 = torch.fft.fft2(gray1)
        f2 = torch.fft.fft2(gray2)
        ...
        B, C, H, W = correlation.shape
        correlation = correlation.view(B, -1)
        max_idx = torch.argmax(correlation, dim=1)
        
        dy = (max_idx // W).float()
        dx = (max_idx % W).float()
        
        # Handle wraparound to allow negative shifts
        dy = torch.where(dy > H / 2, dy - H, dy)
        dx = torch.where(dx > W / 2, dx - W, dx)
        
        return dx, dy

    def warp_tensor(self, img_tensor, flow):
        """Warps the historical tensor buffer forward to match the current camera perspective"""
        B, C, H, W = img_tensor.shape
        xx = torch.linspace(-1.0, 1.0, W, device=self.device)
        yy = torch.linspace(-1.0, 1.0, H, device=self.device)
        grid_y, grid_x = torch.meshgrid(yy, xx, indexing='ij')
        grid = torch.stack((grid_x, grid_y), dim=0).unsqueeze(0).expand(B, -1, -1, -1)
        
        vgrid = torch.empty_like(grid)
        vgrid[:, 0, :, :] = grid[:, 0, :, :] + flow[:, 0, :, :] * 2.0 / max(W - 1, 1)
        vgrid[:, 1, :, :] = grid[:, 1, :, :] + flow[:, 1, :, :] * 2.0 / max(H - 1, 1)
        vgrid = vgrid.permute(0, 2, 3, 1)
        
        return torch.nn.functional.grid_sample(img_tensor, vgrid, mode='bilinear', padding_mode='border', align_corners=True)

    def run_pipeline(self, is_preview=False):
        try:
            active_engine = self.q_mode.get()
            
            if "AI Optical Flow" in active_engine:
                self.init_ai_flow()
            ...
            if is_preview: decoder_out_kwargs['s'] = f'{proc_w}x{proc_h}'
            ...
            if not is_preview:
                ...
                out_kwargs = {
                    'vcodec': 'hevc_nvenc', 'preset': f'p{int(self.p_preset.get())}',
                    'video_bitrate': f'{int(self.q_bitrate.get())}M', 'pix_fmt': 'yuv420p10le',
                    'max_muxing_queue_size': 99999, 'shortest': None
                }
            ...
            with torch.cuda.stream(stream), torch.inference_mode(), torch.amp.autocast('cuda'):
                while True:
                    ...
                    curr = pinned_tensors[idx].to(self.device, non_blocking=True).permute(2,0,1).unsqueeze(0).half()
                    curr_f32 = curr.to(torch.float32)
                    num_elements = min(frame_count, win_size)

                    # --- 1. MOTION ENGINE SWITCHER & WARPING ---
                    if frame_count > 0:
                        if "AI Optical Flow" in active_engine:
                            flow = self.estimate_ai_flow(curr_f32, prev_f32)
                        elif "Global Pan" in active_engine:
                            dx, dy = self.estimate_global_shift(curr_f32, prev_f32)
                            flow = torch.zeros((1, 2, proc_h, proc_w), device=self.device, dtype=torch.float32)
                            flow[:, 0, :, :] = -dx.view(-1, 1, 1)
                            flow[:, 1, :, :] = -dy.view(-1, 1, 1)
                        else:
                            flow = None # Static Camera Engine

                        if flow is not None:
                            for b in range(num_elements):
                                buffer_tensor[b:b+1] = self.warp_tensor(buffer_tensor[b:b+1].to(torch.float32), flow).to(torch.float16)
                    
                    buf_idx = frame_count % win_size
                    buffer_tensor[buf_idx] = curr.squeeze(0)
                    prev_f32 = curr_f32.clone()

                    # --- 2. TEMPORAL MEDIAN & MAD ---
                    if num_elements > 1:
                        median_frame, _ = torch.median(buffer_tensor[:num_elements+1], dim=0, keepdim=True)
                        mad_frame, _ = torch.median(torch.abs(buffer_tensor[:num_elements+1] - median_frame), dim=0, keepdim=True)
                        
                        diff = torch.abs(curr - median_frame)
                        mirage_mask = torch.clamp((diff - (mad_frame * s_params['haze_threshold'])) * s_params['haze_depth'], 0, 1)
                        mirage_mask = apply_gaussian_blur(mirage_mask, kernel_size=7, sigma=2)
                        restored = torch.lerp(curr, median_frame, mirage_mask * s_params['haze_suppression'])
                    else:
                        restored = curr
                    ...
                    if is_preview:
                        ...
                        out_idx = None
                        while not self.stop_requested:
                            try: out_idx = free_writer_queue.get(timeout=1.0); break
                            except Empty: pass
            ...
            if not is_preview:
                ...
                try: encoder.wait(timeout=180) 
                except Exception as e: print(f"Forcing FFmpeg exit: {e}"); encoder.terminate()
                
                output_info = (
                    f"AI Video Master Created Successfully!\n\n"
                    f"Output Resolution: {orig_w}x{orig_h}\n"
                    f"Frame Rate: {fps_str} fps\n"
                    f"Target Bitrate: {int(self.q_bitrate.get())} Mbps\n"
                    f"Tracking Used: {active_engine}\n"
