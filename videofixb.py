...
class HeatWaveUltimateStudio(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("RTX 3090 MASTERING STUDIO v12.1 (AI Optical Flow Final)")
    ...
    def setup_ui(self):
        ...
        self.h_suppress = self.add_smart_slider(t_haze, "Haze Suppress", 0, 1, 0.90, 0, 0, "Strength of mirage removal.")
        ...
        self.q_win = self.add_smart_slider(t_haze, "Stable Window", 3, 30, 9, 3, 0, "AI Aligned Frames for Median.", True)
    ...
    def start_preview(self): 
        ...
        self.preview_display.configure(image=None, text="INITIALIZING AI OPTICAL FLOW...")
        Thread(target=self.run_pipeline, args=(True,), daemon=True).start()

    def start_export(self): 
        if self.input_path: 
            self.is_playing = False; self.preview_display.configure(image=None, text="INITIALIZING AI OPTICAL FLOW...")
    ...
    # --- AI OPTICAL FLOW & WARPING METHODS ---
    ...
    def estimate_flow(self, curr_frame, prev_frame):
        """Calculates optical flow backward from current to previous frame"""
        
        # Scale pixels to[0, 1] range for accurate AI evaluation
        ...
        # Downscale for performance/VRAM if needed
        if scale < 1.0:
            ...
            # RAFT requires dimensions to be a multiple of 8
        ...
        # Pass as two separate arguments for the transforms
        ...
        # Rescale the calculated flow map back to the original video dimensions
        if scale < 1.0:
        ...
        try:
            ...
            with torch.cuda.stream(stream), torch.inference_mode(), torch.amp.autocast('cuda'):
                while True:
                    ...
                    # --- 1. AI OPTICAL FLOW & WARPING ---
                    if frame_count > 0:
                        flow = self.estimate_flow(curr_f32, prev_f32)
                        # Iterative warp for VRAM safety
                    ...
                    if is_preview:
                        ...
                        if frame_count % 5 == 0: 
                            ...
                            stats_text = (
                                ...
                                f"AI Engine Active:   RAFT Optical Flow + Median\n"
